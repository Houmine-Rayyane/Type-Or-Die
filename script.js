...
//This function chooses one word from the list of words declared above 
function getRandomWord() {
    let randomIndex = Math.floor(Math.random() * listWords.length);
    return listWords[randomIndex];
}

//This function makes the word chosen by the function getRandomWord fall vertically
//It also decreases the lives by 1 if the player can't type the word correctly before it disappears from the window 
function moveWord(){
...
//This function verifies if the word typed by the player matches the spelling of any falling word
...
//This function displays the game over window and the final score
function GameOver(){
...
//This function resets the variables socre , lives to initial values and removes the active words from the screen 
function resetGame() {
    Start = false;
